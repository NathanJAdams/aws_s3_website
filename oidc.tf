data "aws_iam_openid_connect_provider" "bitbucket" {
  count = (local.oidc_is_bitbucket && !var.oidc_bitbucket.add_resource) ? 1 : 0
...
resource "aws_iam_openid_connect_provider" "bitbucket" {
  ...
  thumbprint_list = [var.oidc_bitbucket.thumbprint]
}
...
locals {
  ...
  oidc_bitbucket_bare_url          = "api.bitbucket.org/2.0/workspaces/${var.oidc_bitbucket.workspace_name}/pipelines-config/identity/oidc"
  oidc_bitbucket_url               = "https://${local.oidc_bitbucket_bare_url}"
  oidc_bitbucket_audience_variable = "${local.oidc_bitbucket_bare_url}:aud"
  oidc_bitbucket_audience_value    = "ari:cloud:bitbucket::workspace/${trim(var.oidc_bitbucket.workspace_uuid, "{}")}"
  oidc_bitbucket_subject_variable  = "${local.oidc_bitbucket_bare_url}:sub"
  oidc_bitbucket_subject_value     = "{${trim(var.oidc_bitbucket.repo_uuid, "{}")}}:*"
  ...
  oidc_github_subject_value     = "repo:${var.oidc_github.account_name}/${var.oidc_github.repository_name}:*"

  oidc_provider_arn = local.oidc_is_bitbucket
    ? var.oidc_bitbucket.add_resource
